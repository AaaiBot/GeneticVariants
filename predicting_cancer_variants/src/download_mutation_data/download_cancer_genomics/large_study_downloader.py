...
from predicting_cancer_variants.src.common import database_functions
from predicting_cancer_variants.src.common.database_functions import DatabaseHelper
from predicting_cancer_variants.src.download_mutation_data.download_cancer_genomics.downloader import \
    Downloader
...
class LargeStudyDownloader(Downloader, ABC):
    ...
    def get_all_large_studies(self):
        large_studies_sql_command = f'select studyID from ' \
                                    f'{self._configuration.database_table_name_study} where allSampleCount > ' \
                                    f'{self._minimum_number_of_samples_per_study} order by allSampleCount desc '
